...
from acme_ai_sdk import AcmeAISDK, AsyncAcmeAISDK, DefaultAioHttpClient
from acme_ai_sdk._utils import is_dict
...
logging.getLogger("acme_ai_sdk").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[AcmeAISDK]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with AcmeAISDK(base_url=base_url, bearer_token=bearer_token, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncAcmeAISDK]:
